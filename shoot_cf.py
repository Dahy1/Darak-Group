...
ROOT=os.path.join(os.getcwd(),'site'); PORT=8879
def serve():
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        for i,off in enumerate([1600,1250,950]):
            await pg.evaluate(f"window.scrollTo(0,{H-off})"); await pg.wait_for_timeout(650)
            await pg.screenshot(path=f'shots/v5-{i}.png')
