...
async def main():
    async with async_playwright() as p:
        ...
        await m.wait_for_timeout(700)
        info=await m.evaluate("""()=>{const im=document.querySelector('#ohzImpulse .ohz-stack-img'); const cs=getComputedStyle(im); const st=im.parentElement; const scs=getComputedStyle(st);
          return {imgHeightCSS:cs.height, imgPosition:cs.position, top:cs.top,bottom:cs.bottom,inset:cs.inset, objectFit:cs.objectFit, stackHeight:scs.height, stackPosition:scs.position, transform:cs.transform.slice(0,40)};}""")
        print(info)
        await b.close()
asyncio.run(main())
