...
ROOT=os.path.join(os.getcwd(),'site'); PORT=8803
def serve():
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        await pg.goto(f'http://127.0.0.1:{PORT}/about-us/',wait_until='load'); await pg.wait_for_timeout(2200)
        await pg.evaluate("()=>{const p=document.getElementById('ohPreloader');if(p)p.remove();}")
        await pg.wait_for_timeout(1500)
        await pg.screenshot(path='shots/about-hero.png')
        # open lightbox
        await pg.evaluate("()=>{const b=document.querySelector('[data-oha-play]'); if(b) b.click();}")
        await pg.wait_for_timeout(900)
        await pg.screenshot(path='shots/about-lightbox.png')
        # mobile
        m=await b.new_page(viewport={'width':390,'height':844},device_scale_factor=2,is_mobile=True)
        await m.goto(f'http://127.0.0.1:{PORT}/about-us/',wait_until='load'); await m.wait_for_timeout(2200)
        ...
        await m.screenshot(path='shots/about-hero-m.png')
        await b.close()
asyncio.run(main())
