...
ROOT=os.path.join(os.getcwd(),'site'); PORT=8877
def serve():
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        for i,off in enumerate([1500, 1150, 850]):
            await pg.evaluate(f"window.scrollTo(0,{H-off})"); await pg.wait_for_timeout(700)
            await pg.screenshot(path=f'shots/v4d-{i}.png')
        await b.close()
asyncio.run(main())
