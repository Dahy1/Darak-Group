...
ROOT=os.path.join(os.getcwd(),'site'); PORT=8812
def serve():
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        await pg.goto(f'http://127.0.0.1:{PORT}/about-us/',wait_until='load'); await pg.wait_for_timeout(1800)
        await pg.evaluate("()=>{const p=document.getElementById('ohPreloader');if(p)p.remove();}")
        await pg.wait_for_timeout(1800)
        await pg.screenshot(path='shots/newhero-desktop.png')
        m=await b.new_page(viewport={'width':390,'height':844},device_scale_factor=2,is_mobile=True)
        await m.goto(f'http://127.0.0.1:{PORT}/about-us/',wait_until='load'); await m.wait_for_timeout(1800)
        ...
        await m.screenshot(path='shots/newhero-mobile.png')
        await b.close()
asyncio.run(main())
